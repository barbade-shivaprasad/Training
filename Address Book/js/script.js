...
function selectContact(id) {
    let previousElement;
    if (selectedId != "") {
        previousElement = document.getElementById(selectedId);
...
async function setContactDetails() {
    let contact = await DataProviderService.getContact(Number.parseInt(selectedId));
...
function getFormData() {
    ...
    let fullAddress = new Address(InputAddress[0], InputAddress[1], InputAddress[2], Number(InputAddress[3]));
    if (website == "")
        website = null;
...
async function editContact() {
    ...
    let formData = getFormData();
    formData.id = id;
    let currentElement = document.getElementById(selectedId);
...
async function setFormData() {
    displayForm('edit');
    let ele = await DataProviderService.getContact(Number.parseInt(selectedId));
    ...
    document.getElementById('mobile').value = ele.mobile;
    document.getElementById('landline').value = ele.landline;
    document.getElementById('website').value = ele.website;
...
function displayErrorMsg(id, errorType) {
